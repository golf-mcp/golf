...
class JWTAuthConfig(BaseModel):
    ...
    issuer: str | None = Field(
        None, description="Expected JWT issuer claim"
    )
    audience: str | list[str] | None = Field(
        None, description="Expected JWT audience claim(s)"
    )
    algorithm: str = Field(
        "RS256", description="JWT signing algorithm"
    )
...
class OAuthServerConfig(BaseModel):
    ...
    # OAuth server URLs
    base_url: str = Field(
        ..., description="Public URL of this Golf server"
    )
    ...
    allow_token_revocation: bool = Field(
        True, description="Allow token revocation"
    )
...
class RemoteAuthConfig(BaseModel):
    ...
    resource_server_url: str = Field(
        ..., description="URL of this resource server"
    )
