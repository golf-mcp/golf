...
def generate_api_key_auth_components(server_name: str, opentelemetry_enabled: bool = False, transport: str = "streamable-http") -> dict:
    ...
    setup_code_lines = [
        "# Global store for API keys per request/task",
        "# This persists across async boundaries within FastMCP",
        "_api_key_store: Dict[int, Optional[str]] = {}",
        "",
        "def store_api_key_for_task(api_key: Optional[str]):",
        "    \"\"\"Store API key for the current async task.\"\"\"",
        "    task_id = id(asyncio.current_task())",
        "    _api_key_store[task_id] = api_key",
        "",
        "def get_api_key_for_task() -> Optional[str]:",
        "    \"\"\"Get API key for the current async task.\"\"\"",
        "    task_id = id(asyncio.current_task())",
        "    return _api_key_store.get(task_id)",
        "",
        "def cleanup_api_key_for_task():",
        "    \"\"\"Clean up API key for the current async task.\"\"\"",
        "    task_id = id(asyncio.current_task())",
        "    _api_key_store.pop(task_id, None)",
        "",
        "# Context variable for API key storage (fallback)",
        ...
        "                )",
        "            ",
        "            # Store the API key in multiple places for maximum compatibility",
        "            # 1. Context variable (for sync access)",
        "            api_key_context.set(api_key)",
        "            set_api_key(api_key)",
        "            ",
        "            # 2. Task-based store (persists across async boundaries)",
        "            store_api_key_for_task(api_key)",
        "            ",
        "            # 3. Request state (for direct access)",
        ...
        "            return response",
        "        finally:",
        "            # Clean up task-based storage",
        "            cleanup_api_key_for_task()",
        "",
    ]
