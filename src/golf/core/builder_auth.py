...
def generate_api_key_auth_components(server_name: str, opentelemetry_enabled: bool = False, transport: str = "streamable-http") -> dict:
    ...
    setup_code_lines = [
        ...
        "        print(f'[ApiKeyMiddleware] === MIDDLEWARE CALLED for {request.method} {request.url.path} ===')",
        "        print(f'[ApiKeyMiddleware] Headers: {dict(request.headers)}')",
        "        ",
        ...
        "        ",
        "        api_key_config = get_api_key_config()",
        "        print(f'[ApiKeyMiddleware] API key config exists: {api_key_config is not None}')",
        "        if api_key_config:",
        "            print(f'[ApiKeyMiddleware] Config - header_name: {api_key_config.header_name}, prefix: {api_key_config.header_prefix}, required: {api_key_config.required}')",
        "        ",
        "        if debug:",
        "            print(f'[ApiKeyMiddleware] API key config: {api_key_config}')",
        "        ",
        "        if api_key_config:",
        ...
        "            ",
        "            if debug:",
        "                print(f'[ApiKeyMiddleware] Looking for header: {header_name} with prefix: {header_prefix}')",
        "            ",
        ...
        "                    api_key = v",
        "                    print(f'[ApiKeyMiddleware] Found matching header {k}: {v[:30]}...')",
        "                    break",
        "            ",
        "            if debug:",
        "                print(f'[ApiKeyMiddleware] Raw API key found: {api_key is not None}')",
        "            ",
        "            # Process the API key if found",
        "            if api_key:",
        "                print(f'[ApiKeyMiddleware] Processing API key...')",
        "                # Strip prefix if configured",
        "                if header_prefix and api_key.startswith(header_prefix):",
        "                    api_key = api_key[len(header_prefix):]",
        "                    print(f'[ApiKeyMiddleware] Stripped prefix, result: {api_key[:10]}...')",
        "                ",
        "                # Store the API key in request state for tools to access",
        "                print(f'[ApiKeyMiddleware] Storing API key in request.state.api_key')",
        "                request.state.api_key = api_key",
        "                ",
        "                # Also store in context variable for tools",
        "                print(f'[ApiKeyMiddleware] Calling set_api_key()')",
        "                set_api_key(api_key)",
        "                ",
        "                if debug:",
        "                    print(f'[ApiKeyMiddleware] Stored API key (first 10 chars): {api_key[:10]}...')",
        "            else:",
        "                print(f'[ApiKeyMiddleware] No API key found in request')",
        "        ",
