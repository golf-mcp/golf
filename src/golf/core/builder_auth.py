...
def generate_api_key_auth_components(server_name: str, opentelemetry_enabled: bool = False, transport: str = "streamable-http") -> dict:
    ...
    setup_code_lines = [
        "# Enhanced API key storage with multiple mechanisms",
        "class ApiKeyStorage:",
        "    def __init__(self):",
        "        # Request ID based storage (most reliable)",
        "        self._request_store: Dict[str, str] = {}",
        "        # Task-based storage with weak references",
        "        self._task_store: weakref.WeakKeyDictionary = weakref.WeakKeyDictionary()",
        "        # Thread-local storage for sync contexts",
        "        import threading",
        "        self._thread_local = threading.local()",
        "        ",
        "    def store_api_key(self, api_key: Optional[str], request_id: str):",
        "        \"\"\"Store API key using multiple mechanisms.\"\"\"",
        "        # Store by request ID",
        "        if api_key is not None:",
        "            self._request_store[request_id] = api_key",
        "        ",
        "        # Store in thread-local",
        "        self._thread_local.api_key = api_key",
        "        self._thread_local.request_id = request_id",
        "        ",
        "        # Store for current task if in async context",
        "        try:",
        "            task = asyncio.current_task()",
        "            if task:",
        "                self._task_store[task] = (api_key, request_id)",
        "        except RuntimeError:",
        "            pass",
        "        ",
        "    def get_api_key(self, request_id: Optional[str] = None) -> Optional[str]:",
        "        \"\"\"Get API key using multiple fallback mechanisms.\"\"\"",
        "        # Try request ID first (most reliable)",
        "        if request_id and request_id in self._request_store:",
        "            return self._request_store.get(request_id)",
        "        ",
        "        # Try current task",
        "        try:",
        "            task = asyncio.current_task()",
        "            if task and task in self._task_store:",
        "                api_key, _ = self._task_store[task]",
        "                return api_key",
        "        except RuntimeError:",
        "            pass",
        "        ",
        "        # Try thread-local",
        "        if hasattr(self._thread_local, 'api_key'):",
        "            return self._thread_local.api_key",
        "        ",
        "        # Try to get request ID from thread-local and look it up",
        "        if hasattr(self._thread_local, 'request_id'):",
        "            return self._request_store.get(self._thread_local.request_id)",
        "        ",
        "        return None",
        "        ",
        "    def cleanup_request(self, request_id: str):",
        "        \"\"\"Clean up API key for a specific request.\"\"\"",
        "        self._request_store.pop(request_id, None)",
        "",
        "# Global API key storage instance",
        "api_key_storage = ApiKeyStorage()",
        "",
        "# Context variable for request ID (this propagates well)",
        "request_id_context = contextvars.ContextVar('request_id', default=None)",
        "",
        "# Enhanced get_api_key function for tools to use",
        "def get_api_key_for_task() -> Optional[str]:",
        "    \"\"\"Get API key for the current context.\"\"\"",
        "    # Try to get request ID from context",
        "    request_id = request_id_context.get()",
        "    return api_key_storage.get_api_key(request_id)",
        "",
        ...
        "            ",
        "            if debug:",
        "                print(f'[API Key Debug] Looking for header: {header_name}')",
        "                print(f'[API Key Debug] Expected prefix: {header_prefix}')",
        "                print(f'[API Key Debug] Request headers: {dict(request.headers)}')",
        "            ",
        ...
        "                    api_key = v",
        "                    if debug:",
        "                        print(f'[API Key Debug] Found header {k}: {v[:10]}...' if len(v) > 10 else f'[API Key Debug] Found header {k}: {v}')",
        "                    break",
        "            ",
        "            # Check if API key is required and missing",
        "            if api_key_config.required and not api_key:",
        "                if debug:",
        "                    print(f'[API Key Debug] API key required but not found!')",
        "                return JSONResponse(",
        ...
        "                if debug:",
        "                    print(f'[API Key Debug] Stripped prefix, final key: {api_key[:10]}...' if len(api_key) > 10 else f'[API Key Debug] Stripped prefix, final key: {api_key}')",
        "            elif api_key and header_prefix and api_key_config.required:",
        "                # Has API key but wrong format when required",
        "                if debug:",
        "                    print(f'[API Key Debug] API key found but missing required prefix!')",
        "                return JSONResponse(",
        ...
        "                )",
        "            ",
        "            # Store the API key in multiple places",
        "            # 1. Our enhanced storage",
        "            api_key_storage.store_api_key(api_key, request_id)",
        "            ",
        "            # 2. Context variable for request ID",
        "            request_id_context.set(request_id)",
        "            ",
        "            # 3. Golf's set_api_key (for compatibility)",
        "            set_api_key(api_key)",
        "            ",
        "            # 4. Request state (for direct access)",
        "            request.state.api_key = api_key",
        "            request.state.request_id = request_id",
        "            ",
        "            if debug:",
        "                print(f'[API Key Debug] Stored API key with request ID: {request_id}')",
        "                print(f'[API Key Debug] API key available: {api_key is not None}')",
        "        ",
        "        try:",
        ...
        "            return response",
        "        finally:",
        "            # Clean up request-specific storage",
        "            if 'request_id' in locals():",
        "                api_key_storage.cleanup_request(request_id)",
        "",
    ]
