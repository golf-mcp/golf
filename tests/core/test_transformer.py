...
class TestImportTransformer:
    ...
    def test_mixed_import_styles_in_single_import(self, tmp_path):
        ...
        node = ast.Import(names=[ast.alias(name="config", asname=None), ast.alias(name="os", asname=None)])
        result = transformer.visit(node)
    ...
    def test_from_import_transformation(self, tmp_path):
        ...
        node = ast.ImportFrom(
            module="env",
            names=[ast.alias(name="API_KEY", asname=None), ast.alias(name="TIMEOUT", asname=None)],
            level=0,
        )
        result = transformer.visit(node)
    ...
    def test_nested_component_imports(self, tmp_path):
        ...
        node = ast.ImportFrom(module="database", names=[ast.alias(name="connection", asname=None)], level=0)
        result = transformer.visit(node)
...
class TestAbsoluteImports:
    ...
    def test_absolute_imports_preserves_from_imports(self, tmp_path):
        ...
        node = ast.ImportFrom(module="config", names=[ast.alias(name="API_KEY", asname=None)], level=0)
        result = transformer.visit(node)
